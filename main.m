...
function [lmList, handLabel, didFlip, roiUsed] = getHandLandmarks(frame, usePy, cv2, hands, H, W, scale, roi, roiMin)
    ...
    plist = py.list(results.multi_hand_landmarks);
    cells = cell(plist);
    hand0 = cells{1};
    
    pts_np = lms2px(hand0, int32(wS), int32(hS));  
    pts = double(pts_np);                          
    if ndims(pts) ~= 2 || size(pts,2) ~= 2         
        pts = reshape(pts, [], 2);
    end    
